...
class CollectionRepository
    ...
    constructor(
            ...
            return Completable
                .fromAction {
                    database.transaction { dsl ->
                        ...
                        val resourceContent =
                            dsl.select(RESOURCE_LINK.RESOURCE_CONTENT_FK)
                                .from(RESOURCE_LINK)
                                .where(
        ...
        override fun getProjectBySlugAndMetadata(
            slug: String,
                ...
                .fromCallable {
                    database.transactionResult { dsl ->
                        ...
                        if (projectEntity == null) {
                            ...
                            ResourceContainer.load(File(mainDerivedMetadata.path)).use { container ->
                                if (container.manifest.projects.none { it.identifier == sourceCollection.slug }) {
                                    container.manifest.projects =
                                        container.manifest.projects.plus(
                                            project {
                                                sort =
                                                    if (
                        ...
                        val workbookDescriptor =
                            database.workbookDescriptorDao.fetch(
                                sourceCollection.id,
                                projectEntity.id,
        ...
        private fun createResourceContainer(
            source: ResourceMetadata,
            targetLanguage: Language,
        ): ResourceContainer {
            val derivedContainerType =
                when (source.type) {
            ...
            val dublinCore =
                dublincore {
                    ...
                    language =
                        org.wycliffeassociates.resourcecontainer.entity.language {
            ...
            val container =
                ResourceContainer.create(directory) {
                    // Set up the manifest
                    manifest =
                        Manifest(
                            dublinCore,
        ...
        private fun linkDerivativeContent(
            dsl: DSLContext,
            sourceId: Int,
            projectId: Int,
        ) {
