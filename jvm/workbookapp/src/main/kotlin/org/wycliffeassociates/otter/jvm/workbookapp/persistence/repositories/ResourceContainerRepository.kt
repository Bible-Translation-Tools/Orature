    ...
    constructor(
            ...
            return Completable
                .fromAction {
                    database.transaction { dsl ->
                        val languageEntity =
                            languageDao.fetchBySlug(languageSlug, dsl)
                        ...
                        val metadata =
                            dublinCore.mapToMetadata(rc.file, language)
                                .let {
        ...
        override fun updateContent(
            ...
            return Single.fromCallable {
                val rcMetadata =
                    getMetadataForContainer(rc) ?: run {
        ...
        private fun getProjectAsOtterTree(
            book: Collection,
            rc: ResourceMetadata,
        ...
        private fun getMetadataForContainer(rc: ResourceContainer): ResourceMetadata? {
            val language =
                languageDao.fetchBySlug(rc.manifest.dublinCore.language.identifier) ?: run {
                    return null
                }
            val metadata =
                resourceMetadataDao
        ...
        override fun removeResourceContainer(resourceContainer: ResourceContainer): Single<DeleteResult> {
            return Single.fromCallable {
                ...
                database.transaction { dsl ->
                    ...
                    val derivedRcExists =
                        resourceMetadataDao.fetchAll().any {
        ...
        inner class ImportHelper(
            ...
            private fun fetchCollectionFromDb(
            ...
            private fun addCollection(
                ...
                val entity =
                    CollectionMapper().mapToEntity(collection).apply {
                        parentFk = parent?.id
            ...
            private fun importCollection(
                parent: Collection?,
                node: OtterTreeNode<CollectionOrContent>,
            ): Collection? {
                val collection =
            ...
            private fun importContent(
                parent: Collection,
                nodes: List<OtterTreeNode<CollectionOrContent>>,
            ) {
                val entities =
                    nodes
            ...
            private fun linkVerseResources(parentCollection: Collection) {
                ...
                val matchingVerses =
                    contentDao.selectLinkableVerses(
                        primaryContentTypes,
                        helpContentTypes,
                        parentCollection.id,
                        dublinCoreIdDslVal,
            ...
            private fun linkChapterResources(parentCollection: Collection) {
                @Suppress("UNCHECKED_CAST")
                val matchingVerses =
                    contentDao.selectLinkableChapters(
                        helpContentTypes,
                        parentCollection.id,
                        dublinCoreIdDslVal,
                    ) as Select<Record3<Int, Int, Int>>
