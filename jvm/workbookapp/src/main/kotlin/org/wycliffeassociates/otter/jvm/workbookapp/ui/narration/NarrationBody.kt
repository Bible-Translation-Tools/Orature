...
class WaveformLayer() : Drawable {
    ...
    override fun draw(context: GraphicsContext, canvas: Canvas) {
        waveformGenerator.fillAmplitudes(allLocalMinAndMaxSamples)
...
class WaveformGenerator {
    ...
    var incomingAudioRingBuffer = FloatRingBuffer(1920 * 2)
    ...
    fun fillAmplitudes(amplitudes : FloatArray) {
        ...
        while(i < remainingAmplitudes / 2) {
            ...
            for(j in 0 until 229) { // TODO: don't hardcode "229".
                currentSample = existingAudio.getShort().toFloat()
                min = minOf(currentSample, min)
                max = maxOf(currentSample, max)
            }
            amplitudes[pos] = scaleAmplitude(min, 1080.0).toFloat()
            amplitudes[pos + 1] = scaleAmplitude(max, 1080.0).toFloat()
            i++
            pos += 2
        }
    ...
    fun fillIncomingAudioRingBufferWithFakeData() {
        val audioCompressor = PCMCompressor(incomingAudioRingBuffer, 229) // TODO: don't hardcode "229"

        while (incomingAudio.remaining() >= 2) {
            val sampleValue = incomingAudio.getShort().toFloat()
            audioCompressor.add(scaleAmplitude(sampleValue, 1080.0).toFloat())
        }
