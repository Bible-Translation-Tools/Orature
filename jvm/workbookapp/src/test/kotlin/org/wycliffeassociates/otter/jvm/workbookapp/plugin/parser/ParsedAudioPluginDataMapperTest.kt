...
class ParsedAudioPluginDataMapperTest {
    var win =
        File.createTempFile("windows", ".exe").apply {
            setExecutable(true)
            deleteOnExit()
        }
    var linux =
        File.createTempFile("linux", ".exe").apply {
            setExecutable(true)
            deleteOnExit()
        }
    var mac =
        File.createTempFile("mac", ".exe").apply {
            setExecutable(true)
            deleteOnExit()
        }

    val PLUGIN_PLATFORM_TABLE =
        listOf(
            mapOf(
                "os.name" to "Mac OS X",
    ...
    fun testCorrectPluginCreatedForEachPlatform() {
        ...
        val inputParsedPlugin =
            ParsedAudioPluginData(
                "Audacity",
                "1.0.1",
                true,
                false,
