...
class OngoingProjectImporter
    ...
    constructor(
        ...
        private fun projectExists(file: File): Boolean {
            ResourceContainer.load(file).use { rc ->
                rc.manifest.dublinCore.let {
                    val sourceLanguageSlug =
                        it.source.firstOrNull()?.language
                            ?: return false

                    val languageSlug = it.language.identifier
                    val projectSlug =
                        rc.manifest.projects.first().let { p ->
                            projectName = p.title
                            p.identifier
                        }
        ...
        private fun updateTakesImportFilter(
            ...
            val chaptersToImport =
                callback.onRequestUserInput(callbackParam).blockingGet().chapters
                    ?: return false
            ...
            return Single.fromCallable {
                try {
                    ...
                    directoryProvider.newFileReader(resourceContainer).use { fileReader ->
                        ...
                        val workbookDescriptor =
                            workbookDescriptorRepository.getAll().blockingGet().firstOrNull {
        ...
        private fun resetChaptersWithoutTakes(
            fileReader: IFileReader,
            derivedProject: Collection,
            mode: ProjectMode,
        ) {
        ...
        private fun importTakes(
            ...
            val existingTakes =
                projectFilesAccessor.audioDir.walk()
        ...
        private fun takeCopyFilter(path: String): Boolean {
            return takesInChapterFilter?.let { takesInChapter ->
                val takePath =
                    takesInChapter.keys.firstOrNull { filterPath ->
        ...
        private fun findSourceCollection(
            ...
            duplicatedTakes.forEach { takePath ->
                parseNumbers(takePath)?.let { (sig, _) ->
                    getContent(sig, project, metadata)?.let { content ->
                        ...
                        val take =
                            copyDuplicatedTakeToProjectDir(
                                takePath,
                                projectAudioDir,
                                content,
                                manifestProject,
        ...
        private fun importSources(fileReader: IFileReader) {
            ...
            val firstTry: Map<String, ImportResult> =
                sourceFiles
        ...
        private fun importSource(
            fileInZip: String,
            fileReader: IFileReader,
        ): Pair<String, ImportResult> {
            val name = File(fileInZip).nameWithoutExtension
            val result =
                importAsStream(name, fileReader.stream(fileInZip))
                    .blockingGet()
        ...
        private fun copyDuplicatedTakeToProjectDir(
            ...
            val now = LocalDate.now()
            val newTakeNumber =
                takeRepository.getByContent(content, false)
                    .blockingGet()
        ...
        private fun getRelativeTakePath(
            pathInRC: String,
        ...
        private fun syncProjectVersion(
