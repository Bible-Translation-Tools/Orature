...
class Narration @AssistedInject constructor(
    ...
    fun resumeRecording(index: Int) {
        ...
        seek(chapterRepresentation.audioLocationToLocationInChapter(chapterRepresentation.totalVerses[index].lastFrame()))
        writer?.start()
        isRecording.set(true)
    }
    ...
    private fun restoreFromExistingChapterAudio(
        ...
        if (!chapterFileExists && chapter.chunks.value?.any { it.hasSelectedAudio() } == true) {
