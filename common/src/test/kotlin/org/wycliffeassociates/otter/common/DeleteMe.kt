...
class DeleteMe {
    ...
    @Test
    fun util() {
        convertAllMp3InDirectoryToWav(
            File("""D:\misc\generatemarkers\audio"""),
            File("""D:\misc\generatemarkers\audio\wav-with-markers""")
        )
    }
    ...
    private fun convertAudioToWav(inputFile: File): File {
    ...
    fun convertAllMp3InDirectoryToWav(inputDir: File, outputDir: File) {
        require(inputDir.isDirectory) { "Input path must be a directory" }
        if (!outputDir.exists()) outputDir.mkdirs()

        inputDir.listFiles { file -> file.extension.equals("mp3", ignoreCase = true) }?.forEach { mp3File ->
            val wavFile = convertAudioToWav(mp3File)
            val outFile = File(outputDir, mp3File.nameWithoutExtension + ".wav")
            wavFile.copyTo(outFile, overwrite = true)
            wavFile.delete() // clean up temp file
        }
    }
}